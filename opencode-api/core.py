...
def send_review_message(message: str) -> Dict[str, Any]:
    """
    发送 review 消息，自动使用 reviewModel 配置

    与 send_message 的区别：
    - 自动从配置文件的 reviewModel 中读取 modelID/providerID
    - apiKey 由服务端配置，无需在客户端设置
    - 适用于代码审查等需要强模型的场景
    ...
    # 从配置读取 reviewModel（apiKey 由服务端配置，无需传递）
    review_model_config = config.get("reviewModel", {})
    review_model_id = review_model_config.get("modelID", "deepseek-v4-pro")
    review_provider_id = review_model_config.get("providerID", "deepseek")
    ...
    body = {
        ...
        "model": {
            ...
            "providerID": review_provider_id
        }
    }

    api_logger.info("=" * 60)
    api_logger.info(f"📝 Review 消息")
    api_logger.info(f"   modelID: {review_model_id}, providerID: {review_provider_id}")
    api_logger.info(f"   text: {message[:200]}...")
    api_logger.info("=" * 60)
